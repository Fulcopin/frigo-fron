...
const SignatureUploader = ({
  ...
  const isDrawingRef = useRef(false); // Ref para touch listeners nativos
  ...
  /**
   * 🎯 AUTO-CARGAR FIRMA GUARDADA desde localStorage
   * Si el usuario tiene firma guardada en "Mi Firma", cargarla automáticamente
   */
  useEffect(() => {
    // Solo autocargar si NO hay firma ya y si es para el usuario actual
    if (hasFirma || autoLoadedSignature) return;

    const currentUser = JSON.parse(localStorage.getItem('currentUser') || '{}');
    if (!currentUser.username && !currentUser.email) return;

    // Verificar si el puesto coincide con el usuario actual (por email)
    ...
    // Buscar firma guardada en localStorage
    ...
    if (savedSignature) {
      console.log(`✅ Auto-cargando firma guardada para: ${currentUser.nombre || currentUser.username}`);
      
      // Aplicar firma guardada automáticamente
      onFirmaChange({
        ...firmaData,
        firma: {
  ...
  useEffect(() => {
    if (activeTab === 'draw' && canvasRef.current) {
      ...
      // Guardar dimensiones ANTES de modificar
      const rect = canvas.getBoundingClientRect();
      const displayWidth = rect.width || 300;
      const displayHeight = rect.height || 150;
      
      canvas.width = displayWidth * 2; // Mejor resolución
      ...
      // ── Touch listeners NATIVOS con { passive: false } ──
      // React registra touch events como passive, lo que impide preventDefault.
      // Necesitamos listeners nativos para evitar el scroll mientras se dibuja.
      const getTouchCoords = (e) => {
        const r = canvas.getBoundingClientRect();
        if (e.touches && e.touches.length > 0) {
          // Las coordenadas ya están en píxeles CSS (no canvas), NO dividir entre scale
      ...
      const handleTouchStart = (e) => {
        e.preventDefault(); // Ahora SÍ funciona porque passive: false
      ...
      // Cleanup: remover listeners nativos y resetear refs
      return () => {
    ...
    // Cleanup para React StrictMode (cuando activeTab !== 'draw')
    return () => {
      contextRef.current = null;
      isDrawingRef.current = false;
      setIsDrawing(false);
  ...
  // Iniciar dibujo (solo mouse — touch se maneja en useEffect con listeners nativos)
  const startDrawing = (e) => {
    if (!contextRef.current) return;
    e.preventDefault(); // Prevenir scroll en mouse también
  ...
  // Dibujar (solo mouse — touch se maneja en useEffect)
  const draw = (e) => {
  ...
  const saveDrawnSignature = async () => {
    ...
    // Verificar que hay algo dibujado - método más simple y confiable
    const context = canvas.getContext('2d');
    if (!context) {
    ...
    // Método simple: verificar si hay píxeles no blancos
    const imageData = context.getImageData(0, 0, canvas.width, canvas.height);
    let hasContent = false;
    
    // Revisar cada píxel (cada 4 valores = RGBA)
    for (let i = 0; i < imageData.data.length; i += 4) {
      const alpha = imageData.data[i + 3]; // Canal alpha
      if (alpha > 0) {
        hasContent = true;
        break;
      }
