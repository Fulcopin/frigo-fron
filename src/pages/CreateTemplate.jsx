      ...
      <div className="form-section">
        ...
        {template.headerFields.map((field, index) => (
          <div key={index} className="field-item">
            ...
            {/* ═══ LAYOUT ESPECIAL LOTE ENTRANTE EN ENCABEZADO ═══ */}
            {field.type === 'lote_entrante' ? (
              <div style={{ background: '#f0fdf4', border: '2px solid #86efac', borderRadius: '12px', padding: '16px', width: '100%', boxSizing: 'border-box' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px', padding: '8px 12px', background: field.usaApi !== false ? '#dcfce7' : '#f3f4f6', borderRadius: '6px', border: `1px solid ${field.usaApi !== false ? '#86efac' : '#d1d5db'}` }}>
                  <input type="checkbox" id={`h-usaApi-${index}`} checked={field.usaApi !== false} onChange={(e) => updateHeaderField(index, 'usaApi', e.target.checked)} style={{ width: '16px', height: '16px', cursor: 'pointer' }} />
                  <label htmlFor={`h-usaApi-${index}`} style={{ cursor: 'pointer', fontWeight: 600, color: field.usaApi !== false ? '#15803d' : '#4a5568', fontSize: '13px', margin: 0 }}>📡 Usa API Externa (autocompletar desde ERP)</label>
                </div>
            ...
            {element.type === 'lote_entrante' && (
              <div className="body-element-content" style={{ background: '#f0fdf4', border: '2px solid #86efac', borderRadius: '12px', padding: '20px' }}>
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <p style={{ color: '#6b7280', fontSize: '13px', margin: '4px 0 0' }}>Campos para registrar información del lote de entrada. Al llenar el formulario, estos datos se vinculan con el lote seleccionado.</p>
                </div>
                ...
                <div style={{ marginTop: '12px', display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  <button
                    ...
                    title="Vincula automáticamente este lote con TODAS las tablas del formulario"
                  >
                    🔗 Vincular con TODAS las tablas
                  </button>
                  <button
                    ...
                    title="Quitar la vinculación de este lote en todas las tablas"
                  >
                    ✂️ Desvincular todas
                  </button>
                  <span style={{ fontSize: '11px', color: '#6b7280' }}>
            ...
            {element.type === 'table' && (
              <div className="body-element-content">
                ...
                {(template.bodyElements.some(el => el.type === 'lote_entrante') || template.headerFields.some(f => f.type === 'lote_entrante')) && (() => {
                  // Construir lista de todos los lotes disponibles
                  const allLotes = [
