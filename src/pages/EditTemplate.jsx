    ...
    bodyElements: prev.bodyElements.map((el, i) => {
      ...
      return {
        ...
        columns: el.columns.map((col, j) => {
          ...
          // que al Descargar Datos aparecían como una columna aparte: el
          // histórico quedaba partido en dos. Con esta lista, la exportación
          // sabe que son la misma y junta los valores.
          if (property !== 'label') return { ...col, [property]: value };

          const anterior = String(col.label || '').trim();
          const nuevo = String(value || '').trim();
          if (!anterior || anterior === nuevo) return { ...col, label: value };

          const previos = Array.isArray(col.nombresAnteriores) ? col.nombresAnteriores : [];
          // Sin repetir, y sin que el nombre actual quede en su propio historial
          // (pasa al renombrar de A a B y después de vuelta a A).
          const historial = [...new Set([...previos, anterior])].filter(n => n !== nuevo);
        ...
        <div className="form-grid">
          ...
          <div className="form-group full-width" style={{ padding: '14px', background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: '8px' }}>
            ...
            {[
              ...
              { campo: 'ayudaCompletarRango', etiqueta: '🐟 Completar rango', detalle: 'Llena un rango de filas con Especie → Producto.' },
            ...
            {element.type === 'tinas' && (
              <div className="body-element-content" style={{ background: '#f0fdfa', border: '2px solid #99f6e4', borderRadius: '12px', padding: '20px' }}>
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <p style={{ color: '#6b7280', fontSize: '13px', margin: '4px 0 0' }}>Define los grupos de tinas, los campos de medición por ciclo, y cuántos ciclos.</p>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
                  ...
                  <input type="number" min="1" max="10" value={element.config?.cycles || 3}
                    onChange={(e) => updateBodyElement(elementIndex, 'config', { ...element.config, cycles: parseInt(e.target.value) || 3 })}
                  ...
                  {(element.config?.groups || []).map((group, gIdx) => (
                    <div key={gIdx} style={{ background: 'white', border: '1px solid #d1d5db', borderRadius: '10px', padding: '14px', marginBottom: '10px' }}>
                      <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap', alignItems: 'end', marginBottom: '10px' }}>
                        ...
                        <div style={{ flex: 2 }}>
                          <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151' }}>Subtítulo</label>
                      ...
                      <div style={{ marginTop: '6px' }}>
                        <label style={{ fontSize: '12px', fontWeight: 600, color: '#374151' }}>Etiquetas (coma-separadas)</label>
                ...
                <div style={{ marginBottom: '16px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                    <h4 style={{ color: '#0f766e', margin: 0 }}>📏 Campos por Ciclo</h4>
                    <button onClick={() => {
                    ...
                    <div key={fIdx} style={{ display: 'flex', gap: '8px', alignItems: 'end', marginBottom: '6px', background: 'white', padding: '8px 10px', borderRadius: '8px', border: '1px solid #e5e7eb' }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <input type="text" value={field.suffix || ''} onChange={(e) => {
                          ...
                          updateBodyElement(elementIndex, 'config', { ...element.config, fields });
                        }} placeholder="ej: lts, ppm" style={{ width: '100%', padding: '5px 8px', border: '1px solid #d1d5db', borderRadius: '6px', fontSize: '13px' }} />
                      </div>
                      <button onClick={() => {
                ...
                {/* Botones rápidos para agregar tinas a grupos existentes */}
                ...
                <div style={{ background: 'white', border: '1px solid #d1d5db', borderRadius: '10px', padding: '14px', overflowX: 'auto' }}>
                  <strong style={{ color: '#374151', fontSize: '13px', marginBottom: '8px', display: 'block' }}>👁️ Vista previa ({(element.config?.groups || []).reduce((acc, g) => acc + g.count, 0)} tinas × {element.config?.cycles || 3} ciclos):</strong>
                  ...
                  <div key={colIndex} className="field-item">
                    <div className="field-grid">
                      <div className="form-group">
                        ...
                        <input type="text" value={column.label} onChange={(e) => updateColumnInTable(elementIndex, colIndex, "label", e.target.value)} placeholder="Ej: Hora, Temperatura"/>

                        {/* 📝 Nombres con que se guardó antes esta columna.
                            Los formularios ya llenados los tienen adentro, así
                            que al Descargar Datos caían en una columna aparte
                            y el histórico quedaba partido. */}
                        {Array.isArray(column.nombresAnteriores) && column.nombresAnteriores.length > 0 && (
                          <div style={{ marginTop: '6px', background: '#f5f3ff', border: '1px solid #ddd6fe', borderRadius: '6px', padding: '8px 10px' }}>
                            <div style={{ fontSize: '11px', fontWeight: 700, color: '#6d28d9', marginBottom: '4px' }}>
                              📝 Se llamó antes:
                            </div>
                            <div style={{ display: 'flex', gap: '5px', flexWrap: 'wrap', marginBottom: '7px' }}>
                              {column.nombresAnteriores.map((n) => (
                                <span key={n} style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', background: '#fff', border: '1px solid #c4b5fd', borderRadius: '999px', padding: '2px 6px 2px 10px', fontSize: '11.5px', color: '#5b21b6' }}>
                                  {n}
                                  <button
                                    type="button"
                                    title="Olvidar este nombre"
                                    onClick={() => updateColumnInTable(
                                      elementIndex, colIndex, 'nombresAnteriores',
                                      column.nombresAnteriores.filter(x => x !== n)
                                    )}
                                    style={{ border: 'none', background: 'none', color: '#a78bfa', cursor: 'pointer', fontSize: '13px', lineHeight: 1, padding: 0 }}
                                  >✕</button>
                                </span>
                              ))}
                            </div>

                            <label style={{ display: 'flex', alignItems: 'flex-start', gap: '7px', cursor: 'pointer', margin: 0 }}>
                              <input
                                type="checkbox"
                                checked={column.unirNombresAnteriores !== false}
                                onChange={(e) => updateColumnInTable(elementIndex, colIndex, 'unirNombresAnteriores', e.target.checked)}
                                style={{ marginTop: '2px', accentColor: '#7c3aed', cursor: 'pointer' }}
                              />
                              <span style={{ fontSize: '11.5px', color: '#5b21b6', lineHeight: 1.45 }}>
                                <strong>Unir los datos viejos en esta columna</strong> al Descargar Datos.
                                <br />
                                <span style={{ color: '#7c3aed', opacity: .8 }}>
                                  Si se desmarca, los registros anteriores salen en su propia columna.
                                </span>
                              </span>
                            </label>
                          </div>
                        ...
                        <div className="form-group full-width" style={{ background: '#eff6ff', border: '1px solid #bfdbfe', borderRadius: '8px', padding: '12px', marginTop: '8px' }}>
                          ...
                          {(column.detalleCampos || []).map((campo, ci) => (
                            <div key={ci} style={{ display: 'flex', gap: '6px', alignItems: 'flex-end', marginBottom: '7px' }}>
                              ...
                              {campo.type === 'select' && (
                                <div style={{ flex: 2 }}>
                                  <label style={{ fontSize: '10.5px', color: '#64748b', fontWeight: 600 }}>Opciones (coma)</label>
                                  <input
                                    type="text"
                                    value={campo.optionsTexto ?? (campo.options || []).join(', ')}
                                    onChange={(e) => {
                                      const campos = [...(column.detalleCampos || [])];
                                      campos[ci] = { ...campos[ci], optionsTexto: e.target.value, options: e.target.value.split(',').map(x => x.trim()).filter(Boolean) };
                                      updateColumnInTable(elementIndex, colIndex, 'detalleCampos', campos);
                                    }}
                                    placeholder="Kg, Lbs, Unidad"
                                    style={{ width: '100%', padding: '6px 8px', border: '1px solid #93c5fd', borderRadius: '5px', fontSize: '13px' }}
                                  />
                                </div>
                              )}
                                      ...
                                      {(pRow._rowSpan?.[colKey] || 1) > 1 && (
                                        <button
                                          onClick={() => {
                                            ...
                                            // Unhide all rows hidden by this merge
